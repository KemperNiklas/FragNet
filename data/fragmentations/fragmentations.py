...
class HigherLevelGraph(BaseTransform):
    def __init__(self, vocab_size, neighbor: Literal["node", "tree"], higher_edge_features=False):
        ...
        self.frag_rep = FragmentRepresentation(
            vocab_size) if neighbor != "tree" else FragmentRepresentation(vocab_size + 1)
    ...
    def __call__(self, graph):
        ...
        if self.neighbor == "node":
            ...
            for node_id, frag_infos in enumerate(graph.substructures):
                ...
                if len(frag_ids) == 2:
                    ...
                    graph.substructures[node_id].append(
                        (junction_id, self.vocab_size))
                    fragment_types.append(
                        [fragment2type["junction"], len(frag_ids)])
                    fragment_id += 1
        ...
        if not higher_edges:
            ...
            graph.higher_edge_index = torch.tensor(
                higher_edges, dtype=torch.long).T.contiguous()
        ...
        if self.higher_edge_features:
            ...
            for higher_edge_id, higher_edge in enumerate(graph.higher_edge_index.T):
                nodes1 = graph.fragments_edge_index[0,
                                                    graph.fragments_edge_index[1, :] == higher_edge[0]]
                nodes2 = graph.fragments_edge_index[0,
                                                    graph.fragments_edge_index[1, :] == higher_edge[1]]
                join_nodes = [node1 for node1 in nodes1 if node1 in nodes2]
                join_edges = [edge_id for edge_id, (node_a, node_b) in enumerate(
                    graph.edge_index.T) if node_a in join_nodes and node_b in join_nodes]
                join_nodes_list += [[higher_edge_id, join_node]
                                    for join_node in join_nodes]
                join_edges_list += [[higher_edge_id, join_edge]
